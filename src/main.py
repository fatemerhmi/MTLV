...
def run(config, gpu_id=0):
    ...
    if training_type == "ttest":
        ...
        for train_dataloader_s, val_dataloader_s, test_dataloader_s, num_labels_s, train_dataloader_mtl, val_dataloader_mtl, test_dataloader_mtl, num_labels_mtl in dataset_obj(dataset_name, dataset_args, tokenizer_obj, tokenizer_args, head_type, head_args, batch_size, model_cfg, fold): 
            ...
            test_f1_micro, test_f1_macro, test_acc = train(train_dataloader_s, val_dataloader_s, test_dataloader_s, model_singleHead, cfg , use_cuda, "singlehead_cls", fold_i)
            results_singleHead.append([test_f1_micro, test_f1_macro, test_acc])
            ...
            test_f1_micro, test_f1_macro, test_acc = train(train_dataloader_mtl, val_dataloader_mtl, test_dataloader_mtl, model_MTL, cfg , use_cuda, "MTL_cls", fold_i)
            results_MTL.append([test_f1_micro, test_f1_macro, test_acc])
        ...
        _, ttest_acc   =  stats.ttest_rel(results_MTL[:,2],results_singleHead[:,2])

        mlflowLogger.store_metric(f"ttest_f1_mi", ttest_f1_mi)          
        mlflowLogger.store_metric(f"ttest_f1_ma", ttest_f1_ma)          
        mlflowLogger.store_metric(f"ttest_acc", ttest_acc)          
        ...
        _, ttest_acc   =  stats.wilcoxon(results_MTL[:,2],results_singleHead[:,2])
    ...
    #==================================cross validation========================================
        ...
        for train_dataloader, val_dataloader, test_dataloader, num_labels in dataset_obj(dataset_name, dataset_args, tokenizer_obj, tokenizer_args, head_type, head_args, batch_size, model_cfg, fold): 
            ...
            #-------setup training
            test_f1_micro, test_f1_macro, test_acc = train(train_dataloader, val_dataloader, test_dataloader, model, cfg , use_cuda, training_type, fold_i)
